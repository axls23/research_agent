...
from research_agent.utils.groq_models import (
    GroqModelClient,
    GroqModelRegistry,
    quick_generate,
    analyze_topic,
...
async def test_groq_models():
    ...
    try:
        ...
        response = await client.generate_text(
            "Explain quantum computing in simple terms", model="llama-3.1-8b-instant"
        )
        ...
        summary_result = await client.summarize_document(
            sample_text, max_length=100, model="llama-3.1-8b-instant"
        )
...
async def test_agents_with_groq():
    ...
    try:
        ...
        if result["status"] == "completed":
            ...
            print(
                f"[ERROR] Search query formulation failed: {result.get('error', 'Unknown error')}"
            )
        ...
        if result["status"] == "completed":
            ...
            print(
                f"[ERROR] Outline generation failed: {result.get('error', 'Unknown error')}"
            )
...
async def test_model_registry():
    ...
    try:
        ...
        print(
            f"Code generation models: {registry.list_models_by_capability('code_generation')}"
        )
...
async def test_convenience_functions():
    ...
    try:
        ...
        response = await quick_generate(
            "What are the benefits of renewable energy?", model="llama-3.1-8b-instant"
        )
        ...
        analysis = await analyze_topic(
            "Sustainable Agriculture", model="meta-llama/llama-4-scout-17b-16e-instruct"
        )
        ...
        code = await generate_code_quick(
            "A simple web scraper in Python", language="python"
        )
