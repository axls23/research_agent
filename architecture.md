...
This document outlines the system architecture, data flow, and state management for the NEXUS system. The system uses a **LangGraph-based multi-agent architecture** with local-first execution, validation gates, human-in-the-loop interventions, and full auditability.
...
- **Knowledge Representation**:
  - **Vector Store**: Local vectors (Neo4j vector indexes and local embedding models).
  - **Graph Database**: Local Neo4j (for entity and principle relationships).
...
## 2. Pipeline Workflow (Deterministic)
...
graph TD
    %% Nodes
    Ingest[Dark Data Ingestion Node]
    DataProc[Data Processing Node]
    Rosetta[Rosetta Core Node]
    ...
    Val1{Validator<br>(Post-Ingestion)}
    ...
    CheckBacktrack -- Yes (Backtrack) --> Ingest
...
2. **Ingestion**: Tracks local-source records and normalized `PaperRecord` structures.
3. **Data Processing**: Holds text `chunks` extracted from full-text PDFs using tools like Mistral Document AI.
4. **Rosetta + Knowledge Graph**: Builds core-principle mappings and structured `KnowledgeEntity` objects.
...
- **Dark Data Ingestion**: Loads local enterprise artifacts using source patterns and normalizes records.
- **Data Processing**: Extracts text and metadata from selected papers, chunks text, and prepares material for embedding.
- **Rosetta Core**: Translates domain jargon into shared principle terms used across silos.
- **Knowledge Graph**: Generates structured entities and loads them into local Neo4j vector/graph storage.
- **Analysis**: Conducts thematic synthesis, statistical evaluation, and prepares tables/figures.
- **Writing**: Synthesizes extracted findings into a structured markdown brief and can backtrack to ingestion if evidence gaps are detected.
- **Audit Formatter**: Finalizes run artifacts and exports an immutable JSON audit trail of the entire run.
